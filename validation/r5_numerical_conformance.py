#!/usr/bin/env python3
"""Exhaustive R5 numerical conformance against a pinned external-origin witness."""
...
from efso_r5_evaluator import evaluate
from efso_r5_space import enumerate_r5_space

EXPECTED_COUNT = 2187
TOLERANCE = 1e-12
WITNESS = ROOT / "validation" / "witnesses" / "r5_mlp_batch_oracle.py"
SOURCE_REPOSITORY = "DEXTR-GROUP/QWENRNS"
SOURCE_PATH = "validation/r5_mlp_batch_oracle.py"
SOURCE_COMMIT = "4db6a1d314a1d0becb63995927feb90f301eb20e"
SOURCE_BLOB_SHA = "901125629523f473a597c9eeebdb8b6cb26a41d7"
...
def as_payload(state) -> dict[str, object]:
    return {
        ...
        "gate_weight": list(state.gate_weight),
        "up_weight": list(state.up_weight),
        "down_weight": list(state.down_weight),
    }
...
def main() -> None:
    ...
    if observed_blob_sha != SOURCE_BLOB_SHA:
        raise SystemExit(
            f"witness provenance mismatch: expected source blob {SOURCE_BLOB_SHA}, "
            f"observed {observed_blob_sha}"
        )
    ...
    for index, (state, expected) in enumerate(zip(states, reference)):
        ...
        if len(expected) != 1:
            ...
            continue
        error = abs(actual[0] - float(expected[0]))
        max_abs_error = max(max_abs_error, error)
        if not math.isclose(actual[0], float(expected[0]), rel_tol=TOLERANCE, abs_tol=TOLERANCE):
            mismatch_count += 1
            if first_mismatch is None:
                first_mismatch = {
                    "index": index,
    ...
    evidence = {
        "schema_version": "EFSO-R5-NUMERICAL-1",
        "check_id": "R5-NUM-001",
        "reference_id": "QWENRNS-R5-BATCH",
        "finite_space": "R5-DOMAIN-3^7",
    ...
    if mismatch_count:
        raise SystemExit(f"R5 numerical conformance failed: {mismatch_count} mismatches")
    print(output)
